...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if(SONG.stage == null || SONG.stage.length < 1) {
			switch (songName)
			{
				case 'abuse' | 'frostbite' | 'nague' | 'top':
					curStage = 'freeplay';
				case 'Edited' | 'Sunset' | 'spinfish':
					curStage = 'nawe';
				case 'just-a-shoe' | 'one-thingy':
					curStage = 'zapatos';
				case 'hallway' | 'trapped':
					curStage = 'elen';
				case 'smoked' | 'in-the-sky':
					curStage = 'fran';
				default:
					curStage = 'RL';
			}
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if (startTimer != null && startTimer.finished)
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", null, true, songLength - Conductor.songPosition - ClientPrefs.noteOffset);
			}
			else
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")");
